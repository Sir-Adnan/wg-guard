...
(async () => {
  ...
  try {
    ...
    const nativeContext = await browser.newContext({ javaScriptEnabled: false, reducedMotion: 'reduce' });
      ...
      await nativeContext.close();
      stage = 'interface delete confirmation and removal';
      await goto('/interfaces');
      const deletableInterface = page.locator('tr').filter({ hasText: 'awg7' });
      await deletableInterface.locator('[aria-haspopup="menu"]').click();
      await deletableInterface.locator('form[action$="/delete"] button[type="submit"]').click();
