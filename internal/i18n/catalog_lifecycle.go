...
func init() {
	...
	entries := map[string][2]string{
		...
		"pending":           {"An interrupted operation requires recovery; inspect lifecycle.json and docs/operations/lifecycle-recovery.md for that operation.", "عملیات قطع‌شده به بازیابی نیاز دارد؛ lifecycle.json و راهنمای همان عملیات در docs/operations/lifecycle-recovery.md را بررسی کنید."},
		"pending.update":    {"An interrupted update requires recovery; run wg-guard update --recover.", "به‌روزرسانی قطع‌شده به بازیابی نیاز دارد؛ wg-guard update --recover را اجرا کنید."},
		"pending.install":   {"An interrupted installation requires recovery; run wg-guard update --recover, then inspect retained prerequisites before uninstall/reinstall.", "نصب قطع‌شده به بازیابی نیاز دارد؛ wg-guard update --recover را اجرا و پیش از حذف و نصب دوباره پیش‌نیازهای باقی‌مانده را بررسی کنید."},
		"pending.restart":   {"An interrupted restart is pending; retry wg-guard restart --yes after correcting the cause.", "راه‌اندازی مجدد قطع‌شده باقی مانده است؛ پس از رفع علت، wg-guard restart --yes را دوباره اجرا کنید."},
		"pending.restore":   {"An interrupted restore is pending; review the archive and retry wg-guard restore ARCHIVE --retry with its password input if needed.", "بازیابی قطع‌شده باقی مانده است؛ آرشیو را بررسی و wg-guard restore ARCHIVE --retry را با ورودی گذرواژه در صورت نیاز دوباره اجرا کنید."},
		"pending.data":      {"Coordinated data/key recovery is required; run wg-guard restore --recover with the recorded archive's password input if needed.", "بازیابی هماهنگ داده و کلید لازم است؛ wg-guard restore --recover را با ورودی گذرواژه آرشیو ثبت‌شده در صورت نیاز اجرا کنید."},
		"pending.core":      {"Core verification is pending; correct package/module observation or complete the required maintenance reboot, then retry wg-guard core switch recommended --confirm-impact.", "تأیید هسته باقی مانده است؛ مشاهده بسته و ماژول را اصلاح یا راه‌اندازی مجدد میزبان را انجام دهید، سپس wg-guard core switch recommended --confirm-impact را دوباره اجرا کنید."},
		"pending.uninstall": {"An interrupted uninstall is pending; inspect retained ownership and retry wg-guard uninstall with the intended preservation/purge options.", "حذف قطع‌شده باقی مانده است؛ مالکیت منابع باقی‌مانده را بررسی و wg-guard uninstall را با گزینه‌های حفظ یا حذف عمدی دوباره اجرا کنید."},
