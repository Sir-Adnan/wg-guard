...
func init() {
	en := map[string]string{
		"owner.title": "Administrator account", "owner.username": "Admin username (3–32 letters, digits, _ or -)", "owner.password": "Admin password (minimum 10 bytes, hidden)", "owner.confirm": "Confirm password", "owner.mismatch": "Passwords do not match. Run setup again.", "owner.required": "Fresh setup requires --owner-password-file with a private password file (0600).", "owner.check_failed": "Could not verify the administrator account. The listener was not started.", "owner.reused": "Existing administrator retained; credentials were not changed.", "owner.failed": "Administrator creation failed. Check the username and password; the listener was not started.", "owner.ready": "Administrator account is ready. Sign in with the credentials you supplied.", "owner.file": "Password file must be a regular private file (0600), with one nonempty password of at most 4096 bytes.",
		...
		"terminal.back": "  0  Back    q  Cancel", "terminal.exit": "  0  Exit    q  Quit", "terminal.choice": "Choose an action", "terminal.invalid": "Enter 1–%d, 0 to go back, or q to cancel.", "terminal.yes_no": "Enter y or n.", "terminal.secret_long": "Input exceeds 4096 bytes.",
	}
	fa := map[string]string{
		...
		"terminal.back": "  0  بازگشت    q  لغو", "terminal.exit": "  0  خروج    q  خروج", "terminal.choice": "عملیات را انتخاب کنید", "terminal.invalid": "عدد 1 تا %d، برای بازگشت 0 یا برای لغو q را وارد کنید.", "terminal.yes_no": "y یا n را وارد کنید.", "terminal.secret_long": "ورودی بیشتر از 4096 بایت است.",
	}
	for k, v := range en {
		catalogs[En][k] = v
	}
