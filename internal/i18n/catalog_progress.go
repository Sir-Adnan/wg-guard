...
func init() {
	entries := [][3]string{
		...
		{"quiet_log", "Verbose command output is saved to %s", "خروجی کامل فرمان‌ها در %s ذخیره می‌شود"}, {"still_working", "Still working · %ds elapsed", "همچنان در حال اجرا · %d ثانیه گذشته"}, {"core_source", "Preparing reviewed source bundle %s; the first build can take several minutes.", "بسته سورس بررسی‌شده %s آماده می‌شود و ساخت نخست ممکن است چند دقیقه طول بکشد."}, {"core_ready", "Reviewed source bundle %s is ready", "بسته سورس بررسی‌شده %s آماده است"}, {"runtime_build", "Building the local Docker runtime; the first build can take several minutes.", "تصویر محلی Docker ساخته می‌شود و ساخت نخست ممکن است چند دقیقه طول بکشد."},
