...
| Administrator setup and failed-install recovery | implemented + unit/in-memory lifecycle tested: username is explicit with `admin` as default; blank password generates 144-bit/24-character credentials; short and mismatched input retry locally; the show-once credential card follows health and lifecycle completion. Existing owners remain unchanged. Data-changing initial-install failure now opens guided uninstall/reset rather than generic update recovery; real Docker acceptance pending this maintenance gate |
| Private SSH-tunnel login | fixed + unit tested: private exposure resolves to loopback-only `dev` transport, allowing its HTTP session cookie through the tunnel; Nginx/external proxy exposure remains `proxy` and public/direct paths remain HTTPS. Real Docker login acceptance pending this maintenance gate |
| Complete removal | implemented + unit tested: safe removal keeps data, node reset purges data/owned packages but keeps the manager, and `--purge-all` additionally removes fixed WG-Guard config/lifecycle, manager/cache and installer-log paths before the manager exits. Unrelated host services and shared operator resources remain out of scope; real Docker acceptance pending this maintenance gate |
