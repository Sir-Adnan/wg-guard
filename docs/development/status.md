...
| `doctor`: platform/privileges/paths/perms/tools/module/DB integrity/interface drift + peer counts/nft/sysctl/tc/disk/endpoint DNS/TLS cert expiry/NTP (timedatectl)/backup posture; Docker AWG inspection runs in-container while system/network checks stay host-side; runtime errors are distinct from confirmed missing links; honest `skip` per platform; `--fix` reuses the boot orchestration and refuses while the service runs | ✅ implemented + unit tested; historical read-only/repair checks and the Docker-aware healthy/unavailable/recovered runtime matrix verified on Ubuntu 24.04.4 amd64 — [phase8.3.md](phase8.3.md) |
