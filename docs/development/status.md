...
are unit/race tested. Milestone 9.3 adds the `stats.read`-protected `/api/v1/node/telemetry`
contract with nullable metrics, deterministic bounds/order, and synchronized OpenAPI;
...
9.6 adds a mode-aware host `logs` command with bounded tail/since, clean follow cancellation and
local closed-component filtering; exact Docker/native argv and failure/stream bounds are unit
...
| Item | Status |
|---|---|
| Persistent verified bootstrap manager, Update Center and state-aware menu | production verified on Ubuntu 24.04.4 amd64 Docker for fresh install, unchanged-entry reuse, independent changed-commit manager refresh, visible source-build heartbeat, manager status and ordered full update with backup/health/core gate. Interrupted-uninstall dispatch, config-independent recovery, zero-only navigation and safe/full-reset selection are regression tested from a real user transcript and a targeted real-VPS synthetic-journal drill. `sudo wg-guard` is offline; simulated `main` change/repeat, invalid selection, strict refresh and transport fallback are shell-fixture verified. No published-release or real cross-bundle core transition is claimed |
| Administrator setup and failed-install recovery | implemented + unit/in-memory lifecycle tested: username is explicit with `admin` as default; blank password generates 144-bit/24-character credentials; short and mismatched input retry locally; the show-once credential card follows health and lifecycle completion. Existing owners remain unchanged. Data-changing initial-install failure now opens guided uninstall/reset rather than generic update recovery. Real Docker confirmed the `admin` default and in-place short-password retry; generated password display was intentionally not captured on-host |
| Private SSH-tunnel login | fixed + unit tested + real Docker verified: private exposure resolves to loopback-only `dev` transport and a secret-stdin login completed at `/` with HTTP 200 and one session cookie. Nginx/external proxy exposure remains `proxy` and public/direct paths remain HTTPS |
| Complete removal | implemented + unit tested + real Docker verified: safe removal keeps data, node reset purges data/owned packages but keeps the manager, and `--purge-all` additionally removes fixed WG-Guard config/lifecycle, manager/cache and installer-log paths before the manager exits. The final VPS had no owned paths or installer-provided Docker binary; unrelated/shared resources remain out of scope |
...
| `doctor`: platform/privileges/paths/perms/tools/module/DB integrity/interface drift + peer counts/nft/sysctl/tc/disk/endpoint DNS/TLS cert expiry/NTP (timedatectl)/backup posture; Docker AWG inspection runs in-container while system/network checks stay host-side; runtime errors are distinct from confirmed missing links; honest `skip` per platform; Docker `--fix` uses managed restart/startup reconciliation while native direct repair refuses a running service | ✅ implemented + unit tested; historical read-only/repair checks and the Docker-aware healthy/unavailable/recovered runtime matrix verified on Ubuntu 24.04.4 amd64; managed-fix candidate VPS check required before merge — [phase8.3.md](phase8.3.md) |
...
| Host shim (docker mode): mode-aware dispatch via the install state — panel/data commands exec into the container (`docker exec -i`), host commands (`install/update/uninstall/status/doctor/version`) run locally, `serve` refused with compose hints; host-owned doctor delegates only AWG inspection to the container | ✅ implemented + unit tested (routing and diagnostic-runtime selection); base routing and exact Docker-aware doctor candidate verified on the real VPS — [phase8.3.md](phase8.3.md) |
