...
Last updated: 2026-09-09. Phases 8, 8.1 and **8.2 — Secure access & persistent manager** are
complete. Phase 9 is next; its design is complete but implementation has not started.
...
| AUD-005 | high | No single CLI workflow aggregates operational logs across deployment modes | Phase 9 | planned; design contract complete |
...
| AUD-048 | high | Interactive install silently selects username `owner`; a short password is rejected only after deployment/data preparation, leaving an initial-install journal without a useful retry/reset path and causing apparent successful credentials to fail as `admin` | Phase 8.2 maintenance | fixed and automated-test verified: explicit `admin` default, shared backend validation with in-place retry, blank-password secure generation and post-completion show-once handoff, guided cleanup for every interrupted initial install, and distinct node-reset/complete-removal ownership. Updated real Docker acceptance pending |
| AUD-049 | high | Recommended private Docker setup records loopback HTTP as proxy TLS, making the session cookie `Secure`; valid credentials are accepted but the browser cannot return the cookie over the documented SSH-tunnel HTTP URL and falls back to login | Phase 8.2 maintenance | reproduced on Ubuntu 24.04.4 amd64 with a successful login POST followed by loss of the secure cookie. Fixed and unit tested by resolving private exposure to loopback-only dev transport while retaining proxy mode for real HTTPS proxies; updated real Docker login acceptance pending |
