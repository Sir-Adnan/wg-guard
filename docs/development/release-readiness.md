...
| AUD-046 | high | The installed-node bootstrap fast path opens `/usr/local/bin/wg-guard` before resolving `--commit main`, so a repeated one-line command can keep presenting an old manager after GitHub advances | Phase 8.2 maintenance | implemented + automated verified; the selected identity is resolved first, current builds skip artifacts, changed builds atomically replace only the independent manager cache, invalid selections fail, and temporary acquisition failure uses only a verified cache. Targeted Docker VPS acceptance is required before closing this evidence row |
