...
| `S3`, `S4` | u16 scalar, `[Interface]` | Kernel + userspace | Packet compatibility values for cookie and transport messages. | Dumped; zero/removal may require recreation when HPK is active. | Supported; absent from the legacy-safe recommended profile and populated by every advanced generated profile. Both must be at least 12 with HPK. |
...
| `I1`–`I5` | string, `[Interface]`; backend parses signature tags/bytes | Kernel + userspace | Sender/client-local custom packets; parity is not required. | Dumped as text/hex; omission persists. | Supported as explicit advanced opt-in only. Input must be canonical single-line UTF-8 with no control characters so it cannot create a second config directive. Not generated by the safe recommended profile; client warnings remain because known clients differ. |
...
| `RandomTrailers` | boolean `on/off`, `[Interface]` | Kernel + userspace | Packet-shape behavior must only be enabled for clients verified to tolerate it. | Dumped; omission persists. | Supported. Off in legacy `recommended`; on in advanced generated profiles by product policy. Client compatibility remains material. |
| `DisableCookies` | boolean `on/off`, `[Interface]` | Kernel + userspace | Disables the under-load cookie path and weakens DoS protection. | Dumped; omission persists. | Supported. Off in legacy `recommended`; on in advanced generated profiles by product policy. The DoS tradeoff remains visible in the UI. |
...
`S1`–`S4` to be at least the 12-byte nonce size. Negative J values are rejected before they can
cross an integer-width boundary. HPK text must be the canonical padded base64 encoding of exactly
32 bytes. I1–I5 and endpoint values are separately constrained to safe single-line forms before
runtime or client-config serialization; client rendering repeats the text check as a corrupt-row
defense.
...
- **Resilient:** fixed `12/40/180`, `S1..S4=96/180/48/64`, broader bounded timing ranges, and
  fresh H ranges with spans up to 100,000,000.
- **Suggested:** fixed `5/10/50`, `S1..S4=21/28/53/12`, `H1..H4=1/2/3/4`, padding `10-100`,
  rekey `100-120`, rekey timeout `3-7`, reject `150-180`, keepalive `5-15`, and handshake attempts
  `15-20`. A fresh HPK is generated instead of reusing a shared fixed key. When MTU is omitted,
  this policy uses 1280; DNS continues to use the global client setting (default
  `1.1.1.1, 1.0.0.1`).
- **Randomized:** `Jc=4..12`, `Jmin=20..60`, `Jmax=80..240`; S1/S2 are `12..256` with
  `S1+56 != S2`; S3/S4 are `12..64`. H1–H4 are fresh true ranges, each constructed inside its
  own disjoint header band with a positive span no larger than 100,000,000. Padding/timer intervals
  are generated within the bounded policy in `internal/iface/profile.go`, with RejectAfterTime
  starting after RekeyAfterTime.

Performance, Balanced, Resilient, Suggested, and Randomized populate S3/S4, a fresh 32-byte HPK,
all six advanced timing/padding values, RandomTrailers, and DisableCookies. They deliberately leave
I1–I5 empty for iOS compatibility; an administrator can still opt into those signatures through a
custom profile. These policies are differentiated product starting points, not guarantees of DPI
bypass on every network or client.
...
by the canonical generator. The exact policy/profile preview is AES-GCM sealed under the
installation key and bound to that session's CSRF value; a create cannot claim a generated policy
after changing, omitting, replaying across sessions, or forging the preview. Unrelated edits retain
an existing generated classification only while every AWG value is unchanged. REST creation
rejects `preset` plus explicit `obfuscation`. Stored HPKs are never returned by REST or embedded
into edit-page HTML.
...
- `RandomTrailers` panic history (#178) — the upstream default remains `off`; WG-Guard's advanced
  generated profiles deliberately set it `on`, so compatibility must be verified with the chosen
  client while the legacy `recommended` profile remains available.
