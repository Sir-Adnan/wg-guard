...
On interface creation, `preset` accepts `plain`, `recommended`, or `randomized`. It is mutually
exclusive with an explicit `obfuscation` object. Recommended/randomized values are generated by
the same server policy used by the panel, validated as one coherent set, and persisted with the
actual classification. Explicit parameters are classified as `custom`; an omitted profile with
no parameters is `plain`. API responses may therefore report `plain`, `recommended`,
...
the authenticated session's CSRF value. Create accepts `recommended`/`randomized` only when the
submitted fields match the sealed preview; an edit may preserve an existing generated label
without a new token only when every AWG field is unchanged. The REST `preset` path remains wholly
server-generated and never accepts client-supplied values alongside the policy.
